...
$db = mysqli_connect('localhost', 'root', '12345', 'vz');
...
if (isset($_POST['reg_user'])) {
  ...
  if (empty($username)) { array_push($errors, "Необходимо ввести имя пользователя"); }
  if (empty($email)) { array_push($errors, "Необходимо ввести электронную почту"); }
...
// LOGIN USER проверка заполнения
if (isset($_POST['login_user'])) {
    $username = mysqli_real_escape_string($db, $_POST['username']);
    $password = mysqli_real_escape_string($db, $_POST['password']);
  
    if (empty($username)) {
        array_push($errors, "Необходимо ввести имя пользователя");
    }
    if (empty($password)) {
